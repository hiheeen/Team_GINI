...
import { editSecretFeedApi, getSecretDetailApi } from '../../apis/api';
...
function EditSecretPage() {
  ...
  const { data, isLoading } = useQuery(['secretDetailData', id], () =>
    getSecretDetailApi(cookies.access_token, id),
  );
  console.log(data, 'secret detail data');
  if (isLoading) {
    <div>is loading...</div>;
  }
  const onSubmit = (e) => {
    e.preventDefault();
    const formData = {
      category: value.category || data?.data[0].category,
      title: value.title || data?.data[0].title,
      content: value.content || data?.data[0].content,
        ...
        <form onSubmit={onSubmit}>
          <div>
            ...
            <div>
              <textarea
                ...
                placeholder={data?.data[0].content}
          ...
          <div className={styles.category}>
            ...
            <select
              ...
              value={value.category || data?.data[0].category}
            >
          ...
          <div>
            <select
              ...
              value={value.is_secret || data?.data[0].is_secret}
            >
